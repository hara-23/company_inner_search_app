...
def display_select_mode():
    ...
    with st.sidebar:
        ...
        st.markdown("【入力例】")
        st.caption("社員の育成方針に関するMTGの議事録")

        # 「社内問い合わせ」の機能説明
        st.markdown("**【「社内問い合わせ」を選択した場合】**")
        st.info("質問・要望に対して、社内文書の情報をもとに回答を得られます。")
        st.markdown("【入力例】")
        st.caption("人事部に所属している従業員情報を一覧化して")
                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            source_message = utils.build_source_message(
                                message["content"]["main_file_path"],
                                message["content"]["main_page_number"]
                            )
                            st.success(source_message, icon=icon)
                        else:
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                if "page_number" in sub_choice:
                                    ...
                                    st.info(source_message, icon=icon)
                                else:
                    ...
                    if "file_info_list" in message["content"]:
                        ...
                        for file_info in message["content"]["file_info_list"]:
                            if type(file_info) is dict:
                                ...
                                source_message = utils.build_source_message(source, page_number)
                            else:
                                source = file_info
                                source_message = file_info
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        if "page" in llm_response["context"][0].metadata:
            ...
            source_message = utils.build_source_message(main_file_path, main_page_number)
            st.success(source_message, icon=icon)
        else:
...
def display_contact_llm_response(llm_response):
    ...
    if llm_response["answer"] != ct.INQUIRY_NO_MATCH_ANSWER:
        ...
        for document in llm_response["context"]:
            ...
            source_message = utils.build_source_message(file_path, file_info.get("page_number"))
